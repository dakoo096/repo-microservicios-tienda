    ...
    <div class="container mx-auto px-4 py-8 min-h-screen">
        <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
                <h2 class="text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
                    🛍️ <span class="bg-clip-text text-transparent bg-gradient-to-r from-emerald-800 to-green-600">Mis
                        Compras</span>
                </h2>
                <p class="text-gray-500 mt-2">Historial de plantas adquiridas con tu cuenta.</p>
            </div>
        ...
        <div v-else
            ...
            <h3 class="text-xl font-bold text-gray-800 mb-2">
                {{ ventas.length ? 'Sin resultados para este filtro' : 'No encontramos compras' }}
            </h3>
            <p class="text-gray-500 max-w-sm">
                {{ ventas.length ? 'Prueba cambiando el rango de fechas para encontrar lo que buscas.' : `No hay
                registros de compras actualmente` }}
            </p>
        </div>
        ...
        <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0"
            ...
            <div v-if="detalle"
                ...
                <div
                    class="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden transform transition-all scale-100">
                    <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                        <div>
                            <p class="text-xs text-uppercase text-gray-500 font-bold tracking-wider mb-1">COMPROBANTE
                            </p>
                        ...
                        <ul class="divide-y divide-gray-50">
                            <li v-for="p in detalle.productos" :key="p.codigo"
                                ...
                                <div class="flex items-center gap-4">
                                    <div
                                        class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-xl">
                                        🌱
                                    </div>
                                    <div>
                    ...
                    <div class="p-6 border-t border-gray-100 bg-gray-50 flex flex-col gap-2">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-2xl font-black text-emerald-700">${{ detalle.total.toLocaleString()
                            }}</span>
                        </div>
                    </div>
                </div>
...
const cargarCompras = async () => {
    try {
        if (esAdmin) {
            ...
            personaId.value = localStorage.getItem('personaId')
